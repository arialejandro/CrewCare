...
<div class="container-fluid py-4" style="max-width: 1000px;">
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                ...
                <div class="cc-muted small">{{ __('La salida es del departamento: una persona designada reporta la hora y aplica a todos.') }}</div>
            </div>
        </div>
    ...
    {{-- DEPARTAMENTOS: el designado reporta la salida (aplica a todos). Dos toques: hora + guardar. --}}
    @if (empty($deptRows))
    ...
    @else
                    ...
                    <table class="table align-middle mb-0">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center">{{ __('En llamado') }}</th>
                                <th style="width: 260px;">{{ __('Salida del depto') }}</th>
                                <th></th>
                            </tr>
                        </thead>
                        <tbody>
        ...
        {{-- EXCEPCIÓN individual: alguien que salió a otra hora que su equipo. La ASIGNA el designado. --}}
        <div class="cc-form-card mb-4">
            <div class="cc-form-card__body">
                <div class="fw-semibold mb-1">{{ __('Excepción individual') }}</div>
                <div class="cc-muted small mb-3">{{ __('Solo si alguien salió a distinta hora que su departamento. La asigna quien reporta, no la persona. Gana sobre la del depto.') }}</div>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            <form method="POST" action="{{ route('outs.ind.destroy', $io->id) }}"
                                                  data-confirm="{{ __('¿Quitar esta excepción?') }}">
                                                @csrf
                ...
                <form method="POST" action="{{ route('outs.ind.store') }}" class="row g-2 align-items-end">
                    ...
                    <div class="col-6 col-md-3">
                        <button type="submit" class="btn btn-primary btn-sm w-100">{{ __('Asignar salida') }}</button>
                    </div>
                </form>
            </div>
        </div>

        {{-- PEGAR MENSAJE del grupo (mismo servicio que usará el bot). Designado / producción. --}}
        @if ($canDesignate)
            <div class="cc-form-card">
                <div class="cc-form-card__body">
                    <div class="fw-semibold mb-1">{{ __('Pegar mensaje del grupo') }}</div>
                    <div class="cc-muted small mb-2">{{ __('Ej.: «Cámara 18:30». Confirma lo que entendió; ante la duda, avisa qué no entendió.') }}</div>
