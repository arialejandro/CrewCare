...
<div class="container py-4" style="max-width: 1040px;">
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h1 class="h4 fw-bold mb-0">Posiciones</h1>
                <div class="cc-muted small">Catálogo de posiciones, su departamento, jefatura (HOD) y orden.</div>
            </div>
        </div>
    </div>
    ...
    <div class="cc-form-card">
        <div class="cc-form-card__head">
            ...
            <div class="cc-form-card__titles">
                <h2 class="cc-form-card__title">Agregar posición</h2>
                <p class="cc-form-card__sub">Define el puesto, su departamento y cómo se ordena en el llamado.</p>
            </div>
        </div>
        <div class="cc-form-card__body">
            <form method="POST" action="{{ route('crearpositions') }}">
                @csrf
                <div class="row g-3">
                    ...
                    <div class="col-6 col-md-2">
                        <div class="cc-field mb-0">
                            <label class="cc-label">HOD</label>
                            <div class="form-check pt-1">
                                <input class="form-check-input" type="checkbox" value="1" id="is_hod" name="is_hod"
                                       {{ old('is_hod') ? 'checked' : '' }}>
                                <label class="form-check-label" for="is_hod">Jefatura</label>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="cc-form-card">
        <div class="table-responsive">
            <table class="table cc-cat-table cc-stack align-middle mb-0">
                <thead>
                    <tr>
                        <th scope="col" class="ps-3">Posición</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td data-label="Posición" class="ps-3 cc-cat-name">{{ $pos->name }}</td>
                        <td data-label="Departamento">{{ $pos->department->name ?? '—' }}</td>
                        <td data-label="HOD">
                            @if($pos->is_hod)
                                <span class="cc-chip cc-chip--brand">HOD</span>
                            @else
                                <span class="text-muted">—</span>
                            @endif
                        </td>
                        <td data-label="Orden">{{ $pos->sort_order }}</td>
                        ...
                        <td class="text-end pe-3">
                            <div class="d-inline-flex gap-2">
                                <a class="cc-cat-act" href="{{ route('editpositions', $pos->id) }}"
                                   title="Editar" aria-label="Editar">
                                    @include('componentes._icon', ['name' => 'pencil', 'class' => 'cc-ico-16', 'label' => null])
                                </a>
                                @if($pos->active)
                                <form method="POST" action="{{ route('desactivarposition', $pos->id) }}"
                                      onsubmit="return confirm('¿Desactivar esta posición?');" class="d-inline">
                                    @csrf
                                ...
                                @else
                                <form method="POST" action="{{ route('activarposition', $pos->id) }}" class="d-inline">
                                    @csrf
                    ...
                    <tr>
                        <td colspan="6">
                            <div class="cc-cat-empty">
                                @include('componentes._icon', ['name' => 'id-card', 'class' => 'cc-ico-24 d-block mx-auto mb-2', 'label' => null])
                                <div class="fw-semibold">No hay posiciones registradas.</div>
                                <small>Agrega la primera con el formulario de arriba.</small>
                            </div>
                        </td>
                    </tr>
                    @endforelse
