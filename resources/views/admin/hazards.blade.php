...
<style>
    ...
    /* Tarjeta: hereda vidrio global de .card; hover elevado. */
...
<div class="container-fluid py-4 px-3 px-md-4">
    ...
    @php
        use Carbon\Carbon;
        $grouped = $hazardNotifications->sortByDesc('date_observed')->groupBy(function($item) {
    ...
    <div class="cc-idx-head">
        <div>
            ...
            <h1 class="cc-idx-title">Actos Inseguros</h1>
            <p class="cc-idx-sub">Notificaciones de acciones inseguras observadas.</p>
        </div>
        <div class="d-flex align-items-center gap-2 flex-wrap">
            <span class="cc-idx-count">
                @include('componentes._icon', ['name' => 'shield-alert'])
                {{ $hazardNotifications->total() ?? $hazardNotifications->count() }} <span>registros</span>
            </span>
            {{-- ACCIÓN PRIMARIA: crear (antes duplicada en el sidebar). --}}
            @can('hazards.create')
            <a href="/hazardnotification" class="cc-idx-cta">
                @include('componentes._icon', ['name' => 'plus']) Nuevo acto
            </a>
            @endcan
        </div>
    </div>
    ...
    @forelse ($grouped as $month => $reports)
        ...
        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
            ...
            @endphp
            <div class="col">
                <a href="{{ route('hazard_notifications.show', $report->id) }}" class="text-decoration-none d-block h-100">
                    <div class="card report-card border-0 rounded-3 h-100">
                        ...
                        <div class="card-body">
                            ...
                            <p class="rc-body-line mb-0 small"><strong>Acto inseguro:</strong> {{ Str::limit($report->description_hazard_unsafe_act, 34) }}</p>
                        </div>
                    </div>
                </a>
            </div>
    ...
    @empty
        <div class="card border-0">
            <div class="card-body cc-idx-empty">
                @include('componentes._icon', ['name' => 'shield-alert', 'label' => 'Sin notificaciones'])
                <h5>No hay notificaciones registradas</h5>
                <p class="small mb-0">Las acciones inseguras observadas aparecerán aquí.</p>
            </div>
        </div>
    @endforelse

    <div class="mt-4 d-flex justify-content-center">
        {!! $hazardNotifications->links() !!}
    </div>
</div>
@endsection
