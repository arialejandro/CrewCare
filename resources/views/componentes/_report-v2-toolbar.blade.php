...
@php
    ...
    // $editRoute OPCIONAL: si viene, muestra un botón "Editar" (lo usa hoy el PAE para versionar).
    $tbEdit    = isset($editRoute) ? $editRoute : null;
    $tbEditL   = isset($editLabel) ? $editLabel : ($tbEn ? 'Edit' : 'Editar');
@endphp
<div class="toolbar">
  @if($tbBack)
  ...
  @endif
  @if($tbEdit)
  <a class="tb" href="{{ $tbEdit }}">@include('componentes._icon', ['name' => 'pencil', 'class' => 'w-4 h-4']) {{ $tbEditL }}</a>
  @endif
