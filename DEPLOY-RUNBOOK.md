...
- **`CREWCARE_CSP_ENFORCE` (default `false`)** — el interruptor del BLOQUEO de la CSP, que ya está **COMPLETA**. Con `true` la app emite UNA sola `Content-Security-Policy` (y NADA en Report-Only) que bloquea toda la política: `script-src`/`style-src` = `'self' 'nonce-…'` (el middleware estampa el nonce en cada `<script>`/`<style>` propio; lo inyectado sin nonce se bloquea), `style-src-attr 'unsafe-inline'` (los `style=` en atributo se permiten, no se reescriben), `font-src 'self' data:` (Google Fonts autoalojado, gstatic fuera), `img-src 'self' data: blob:`, `object-src 'none'` + `base-uri 'self'`, `default-src/frame-ancestors/connect-src 'self'`. Con enforce OFF, esa misma política va como `Content-Security-Policy-Report-Only` (solo mide). El **default committeado es reporte** (`false`); el owner lo pone en `true` por entorno cuando toque. Test juez: `php artisan dusk --filter=CspBlockingJudgeTest`.
...
**HTTPS/HSTS (solo producción):** la app fuerza https (`URL::forceScheme`) + redirige http→https + HSTS (middleware `SecurityHeaders`). Requiere que el reverse-proxy TLS mande `X-Forwarded-Proto` — `TrustProxies` confía `*` (la app en el VPS solo es alcanzable por el proxy; si no fuera así, acota a la IP del proxy). **El local sigue en http, sin cambios.** La CSP COMPLETA (script/style/style-attr/font/img/object/base/connect) ya se pasa a **BLOQUEO** con `CREWCARE_CSP_ENFORCE=true` (ver arriba); el default committeado es REPORTE. Ya no queda nada "por bloquear en otra fase": enforce O reporte, la misma política entera.
