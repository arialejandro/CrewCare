...
### 2026-08-09 — ⚠️ CORRECCIONES PENDIENTES (feedback del owner, pre-compact) — NO resueltas aún
- **A) LOS REPORTES SIGUEN CORTANDO.** El cambio a Carta (`fed2d2ed`, `@page{size:letter}` en todo) **NO resolvió los cortes** — el owner sigue viendo texto cortado en la impresión y "no sabe si es por el tamaño". → **PENDIENTE post-compact:** el `@page size` NO era (o no era sólo) la causa. Investigar a fondo el mecanismo de saltos. **CLAVE: no puedo ver la impresión headless** → pedir al owner que señale QUÉ elemento concreto corta (¿una `.tbl`? ¿un `.wnote`? ¿una `.sec` alta?) y en QUÉ reporte, y qué papel tiene seleccionado su diálogo de impresión (el diálogo puede sobrescribir `@page size`). Hipótesis a probar: (a) filas de tabla altas que Chrome parte (convertir esas tablas a divs con `break-inside:avoid`, como el checklist del acta de ambulancia); (b) `break-inside:avoid` no honrado en algún átomo; (c) el papel real del diálogo ≠ letter. Ver [[doc-hero-band-homologation]] gotchas #2/#3.
- **B) HISTORIAL MÉDICO: la impresión NO debe ser como los demás documentos.** El subagente (`1abaf634`) creó una vista standalone sobre el chrome v2 (`componentes/historiamr-print`), pero el owner quería **conservar el look de la vista actual del historial** (`componentes/historiamr`), sólo AJUSTARLA para que imprima/se vea bien (arreglar el truco frágil de `visibility:hidden`). → **PENDIENTE post-compact:** deshacer el enfoque chrome-v2 para el historial (revertir/retirar `historiamr-print` + su ruta `/historialWR/{id}/imprimir`, y regresar el botón a su comportamiento) y en su lugar arreglar la IMPRESIÓN de la vista `historiamr` EXISTENTE (CSS de impresión propio, sin rehacerla como reporte). Ver [[health-record-module]].
- **Estado:** ninguna de las dos tocada aún (el owner pidió compactar). Todo lo demás del lote sí quedó (imágenes/notas Wrap, fotos permisos, accidente demo). El resto del feedback (validar en navegador) sigue en pie.
