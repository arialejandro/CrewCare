...
### 2026-08-11 — 🧱⬆️ Upgrade Etapas 0-2: Laravel 8 → 10 en PHP 8.1 (475 verde) — commits `6447f9fe`, `6a2623f3`, `d0e3e932`
- **Estado:** Hecho hasta **Laravel 10.50.2 / phpunit 10 / PHP 8.1**. Rama `upgrade/laravel-13`. **Suite 475 verde** en cada compuerta (migrate:fresh+seed + view:cache + rutas). **Siguiente = Etapa 3 (L11), que exige PHP 8.3** (lo instala el owner). Breakings de L10: alias Spatie `Middlewares`→`Middleware` (v6) y la migración de permisos (v6 retiró `PermissionRegistrar::$pivotPermission/$pivotRole` → se leen de config, esquema idéntico); phpunit.xml migrado al schema 10. Quedan **28 deprecated** (data providers no-estáticos) a limpiar antes de phpunit 11.
