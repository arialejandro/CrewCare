...
### 2026-08-11 — 🧱⬆️ Upgrade Etapas 0-3: Laravel 8 → 11 (PHP 7.4 → 8.3) — 475 verde en cada salto
- **Estado:** Hecho hasta **Laravel 11.55.0 / phpunit 11 / PHP 8.3.33**. Rama `upgrade/laravel-13`. Commits: `6447f9fe` (0·$dates), `6a2623f3` (1·L9), `d0e3e932` (2·L10), `2c942231` (data providers static), `42dfe35a` (3·L11). **Suite 475 verde** en CADA compuerta (migrate:fresh+seed + view:cache + rutas), sobre PHP 8.1 (L9/L10) y **PHP 8.3** (L11). **PHP 8.3.33 TS x64 instalado en laragon** (`C:\laragon\bin\php\php-8.3.33-...`; ⚠ `extension_dir` ABSOLUTO + `zip`/`sodium` habilitados — el relativo rompía composer desde el repo).
...
- **Siguiente = Etapa 4 (L11→L12→L13)** en PHP 8.3. **Riesgo #1 (sello SHA):** intacto en los 3 saltos (475/475, incluidos los tests de alteración).
