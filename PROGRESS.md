...
### 2026-09-16 — 🐛 El service worker se comía los formularios + 🎬 módulo TECH SCOUT (Locaciones)
- **Estado:** ✅ todo en producción. Suite de documentos y seguridad: **138/138 en verde**.

- **🔥 EL BUG DEL DÍA — el service worker se comía el CUERPO de todos los POST y PUT.** Su manejador de `fetch` corría para TODAS las peticiones y las reemitía con `fetch(req)`; en ese viaje **se pierde el cuerpo**. El servidor recibía la cabecera `multipart/form-data` con su boundary y **cero campos**. Dos síntomas que parecían no tener relación y eran lo mismo: los formularios con archivo llegaban VACÍOS (el owner no pudo guardar ni una nota, con el formulario lleno) y **cualquier PUT daba 405** (Laravel lee el método real de `_method`, que viaja EN EL CUERPO: sin cuerpo se queda en POST). **Afectaba a TODA la app**, cualquier formulario, en cualquier dispositivo con el SW registrado — se registra solo al visitar.
  - 🔑 **Lo que lo destrabó fue una línea de log.** Se perdió una tarde en hipótesis falsas: se descartaron uno por uno el anidamiento de `<form>`, `cc-photo-auto`, el HTML generado y la configuración de PHP. Nada de eso era. En cuanto la validación registró **QUÉ había llegado** (`{"campos":[],"note_len":0,"tiene_file":false,...}`), la causa fue evidente en un minuto. **Una validación que rechaza tiene que dejar dicho qué recibió.**
  - Fix: `if (req.method !== 'GET') { return; }` — la Cache API no soporta no-GET, así que interceptarlos nunca aportó nada. Más `clients.claim()`, o la corrección no llega a las pestañas ya abiertas (en un iPad de set, puede ser nunca). Guardia `ServiceWorkerTest`, verificado en los dos sentidos. 🪤 Su 1ª versión fallaba con el código CORRECTO porque buscaba el texto dentro de su propio comentario → quita comentarios antes de mirar, igual que el del `mousedown`.

- **📸 El borrador de fotos del scouting: faltaba la mitad que RECUPERA.** Lo desplegado el 15 subía las fotos al capturarlas… y NO había forma de que volvieran a un scouting: `create()` calculaba los borradores y la vista los ignoraba. El owner capturó 27 fotos, guardó, y el scouting salió **VACÍO** con las 27 vivas en disco y sin dueño. Segundo eslabón: al guardar, el borrador se borraba **a ciegas**, destruyendo la única referencia a esos archivos. **Guardar sin poder recuperar es peor que no guardar: promete una red que no existe.** Fix: endpoint de recuperación + el borrador sobrevive si sus fotos no se usaron. Las 27 se recuperaron a mano (cero huérfanas). ⚠ **Se probó el guardado y NUNCA el recuperado** — un mecanismo de recuperación sólo está probado cuando se prueba recuperando.

- **🎬 MÓDULO TECH SCOUT** (`tech_scouts` + `tech_scout_notes`) — el scouting técnico de Locaciones: foto + qué hay que resolver. Sustituye el trabajo a mano (fotografiar, anotar en libreta, descargar fotos, pegarlas en Word, reescribir). NO es el Scouting H&S: aquél evalúa riesgos, se sella y tiene valor probatorio; éste es de trabajo y se edita a diario.
  - **CONCURRENCIA RESUELTA POR EL MODELO, no por candados.** Dos scouters recorren la misma locación a la vez. Si fuera UN formulario que ambos guardan, el segundo pisaría al primero sin error ni aviso. Por eso el documento sólo tiene cabecera y todo lo demás son NOTAS independientes: **colaborar es añadir**. El "tiempo real" se resolvió con consulta periódica, no WebSockets: son 2 personas y montar un demonio en el VPS sería pagar por algo que nadie nota.
  - **Lo único obligatorio es la LOCACIÓN**, y la primera nota se emite en el mismo acto que crea el documento. Obligar a completar un panel antes de dejar capturar es lo que hace que la gente vuelva a la libreta.
  - Decisiones del owner fijadas por pruebas: el pie lo firma **Locaciones** (departamento, no persona); **el autor NO se imprime** (dato interno para que los scouters se entiendan); editar **deja marca** (sostiene el «si no está en las notas, no se pidió»); la hora va **debajo** de la foto, nunca encima; orden **cronológico** (agrupar por área no generaliza: una bodega no tiene los cuartos de una casa); etiqueta del **guión**, no del espacio.
  - 🪤 **No se trajo `complexity`**: es un juicio de RIESGO y ese juicio vive en el documento que se sella. Duplicarlo en uno editable invitaría a que los dos digan cosas distintas de la misma locación.
  - Se rehízo **3 veces** la pantalla en el día (lista → paneles → una sola pantalla). Lección: con una pantalla nueva, enseñarla antes de construir encima.

- **🪤 Fragilidades latentes que este módulo destapó:**
  - `btn-crew-*` se define en un parcial que hay que **incluir**; sin él los botones caen al `.btn` pelado y quedan **invisibles** sobre el tema oscuro.
  - El test del visor de bitácora clínica asertaba `"#42"` — el RESPALDO que pinta la vista sólo cuando el paciente no es un usuario conocido. Al crecer la suite el autoincremento pasó de 42 y la prueba se cayó **pareciendo una fuga del filtro en datos clínicos**. No lo era; se comprobó con datos, no por descarte.
  - **Una migración defensiva no se puede ensayar con `--pretend`**: en ese modo los `select` no se ejecutan, `hasTable` devuelve falso y la migración se sale antes de generar SQL. Verificar las columnas directamente.

- **🔒 TECH SCOUT · cierre del módulo: el departamento como criterio de acceso, el refresco, y el teléfono.** (`LocationsAccess`, `EnsureLocationsDepartment`, alias `locaciones`, `_notes-grid.blade.php`, `cc-techscout-refresh.js`)
  - **El acceso lo decide el DEPARTAMENTO, no el permiso.** Pedido del owner: «que eso sólo lo vea quien esté en el departamento de Locaciones, no importa su puesto». Es la primera vez en la app que una puerta NO la abre un permiso: el Tech Scout es la libreta de trabajo de Locaciones, así que un P.A. del departamento entra y un gerente de otro no, aunque tenga `locations.create` y más rango. Las rutas salieron del grupo `permission:locations.create` a uno propio. Emparenta con `TransportAccess` pero **sin la parte híbrida**: allá el permiso también abre; aquí no. Única excepción: **super-admin** —hoy en producción el owner es el único usuario y no está en ningún departamento; sin esa salvedad el cambio lo dejaba fuera de su propio módulo el día que se sube—.
  - 🪤 **Sin memo estático, a propósito.** Tentaba cachear la respuesta por id de usuario (el sidebar la pide dos veces). Un `static` así ya nos mordió: sobrevive entre pruebas del mismo proceso y responde por un usuario que ya no es el mismo. **Un falso "sí" aquí es el aislamiento roto en silencio.**
  - Las pruebas de aislamiento se verificaron **rompiéndolas**: con la guarda anulada, 2 en rojo. Es la única forma de saber que una prueba de seguridad prueba algo — ya pasamos por dos que iban en verde con el candado abierto.
  - **Refresco periódico** (20 s) para los dos scouters: pregunta por la rejilla, compara una FIRMA (`data-ts-sig` = cuántas notas y cuál fue el último cambio) y sólo reemplaza si cambió. Devuelve **HTML de la misma plantilla** que pinta la pantalla, no JSON: con JSON habría que repintar en JavaScript y esa segunda copia se desincroniza (la lección del día). 🔑 **No toca nada si hay un editor abierto o el cursor está en la rejilla**, y lo vuelve a comprobar al volver la respuesta: cambiar el HTML bajo los dedos de alguien le borra lo escrito. Verificado en navegador: la nota del otro entra sola; con el editor abierto, 28 s sin pisar nada.
  - **Para el teléfono, que es lo que traen los scouters** (el owner usa iPad, ellos no): cintillo de una línea en vez de la banda —tercera versión de esa cabecera, «no debe ser tan protagonista»—, **Agregar nota ARRIBA de los paneles** (era pasar tres tarjetas con el pulgar antes de poder anotar), paneles **todos plegados** al abrir, filas de viabilidad/acuerdos apiladas, fechas de dos en dos y botones a todo el ancho.
  - 🪤 **Los paneles "plegables" nunca plegaron.** `_collapsible-sections` engancha tarjetas con `.card-header` + `.card-body` como hijos directos; el Tech Scout usaba `<div class="card p-3">` con un `<h2>` suelto, así que el componente salía sin hacer nada y **nadie lo notó** porque en escritorio la pantalla cabe igual. Al maquetarlo como las tarjetas del Scouting H&S empezó a funcionar — y de paso se homologó el aspecto. Modo nuevo `data-cc-sections="closed"` (todas cerradas salvo las que traen error).

- **📴 LA NOTA SE GUARDA EN EL DISPOSITIVO ANTES DE TOCAR LA RED.** (`cc-techscout-note.js`, `cc-drafts.js#enqueueNow`, `idempotent` en las 2 rutas POST del módulo) Contexto: el owner va mañana a un **penal**. «Cualquier imagen perdida es un problema grave… si algo dentro del penal falla, como la red.»
  - **La regla, y en qué se diferencia de lo que ya había.** Los 5 reportes con envío diferido encolan SÓLO cuando `navigator.onLine` dice que no hay red. Ese "sí hay red" **miente a diario**: wifi de cortesía que no pasa un byte, un sótano con una barra, la red de un penal. Cuando miente, el POST se queda colgado y si en ese momento muere la pestaña **la foto no existía en ningún sitio**. Aquí se encola SIEMPRE —con red o sin ella— y la cola envía enseguida o cuando vuelva la señal. La foto existe en el dispositivo desde el segundo cero.
  - Cubre las DOS pantallas: la captura de notas y el ALTA (que lleva portada + primera nota). Idempotencia por llave en ambas rutas: salir del penal con 20 notas encoladas no puede dejar 40.
  - **Seguro al cerrar**: si hay foto elegida o texto escrito SIN enviar, cerrar pregunta. Con el formulario vacío NO estorba — un aviso que salta siempre se aprende a ignorar siempre.
  - 🪤 **Un agujero que habría perdido notas en silencio:** el rechazo de nota vacía era `back()->withErrors()`, o sea **302**. Para la cola un 3xx es ÉXITO (el store redirige al crear) → habría borrado el borrador dando la nota por buena, **con su foto**. Ahora lanza `ValidationException`: 422 para quien espera JSON, redirección de siempre para el navegador. Fijado por prueba.
  - 🪤 **Y otro de carrera:** `attach()` registra su propio listener de submit ANTES y en línea BORRA el borrador en curso. Encolar con ese mismo id dejaba su borrado asíncrono llegando DESPUÉS del guardado, llevándose la nota recién encolada. Cada nota pide **id nuevo**.
  - ⚠ El comentario de cabecera de `cc-drafts.js` decía «el borrador NO captura `<input type=file>`» — **era falso** desde que se añadió `collectFiles` (el encolado sí lleva fotos; el autoguardado de texto, no). Corregido: una nota que miente cuesta más que ninguna.
  - **Verificado en navegador, las dos mitades**: (1) con la red MUERTA pero `onLine=true`, la nota queda en IndexedDB con su JPEG; (2) **recargando la página** —como si se matara la pestaña— la cola la envía sola y la foto aparece en disco con su tamaño intacto. También el caso `onLine=false` y el alta completa sin señal (portada + primera nota, ambas fotos). *Esta vez se probó recuperando.*

- **🔒 Quien está en el panel SÓLO por Locaciones ve lo suyo y nada más.** (`User::hasPanelPermission()`, `$__soloLoc` en el sidebar) Al abrir el panel por pertenencia al departamento, el menú empezó a ofrecerle secciones que un `crew` nunca había visto porque nunca había tenido menú. El owner señaló dos: **Panel SFX** («no le sirve a locaciones, no es viable en este proyecto») y **Contratos** («tampoco es de su departamento»). Se aplica la regla que él mismo fijó —«el aislamiento hacia los demás departamentos es de locaciones solamente»—. El autoservicio (reportar un accidente, un peligro, sus documentos) se queda: eso sí es suyo. Con prueba por los dos lados, verificada rompiéndola.
  - Se descartó **apagar la bandera `sds_sfx`**, que era el atajo: en producción ese catálogo TIENE contenido (53 consumibles, 25 tipos de efecto) y apagarla se habría llevado por delante material de H&S que el owner sí consulta.
  - 🔴 **REPORTADO, NO CERRADO: la ruta del Panel SFX en vivo (`/sfx`, `/sfx/start`, `/sfx/stop`) no pide NINGÚN permiso** — sólo `auth`. Cualquiera con sesión la abre escribiendo la URL, y desde ahí se dispara de verdad. Esconder el enlace **no es cerrar la puerta**. No se tocó de madrugada porque elegir mal el permiso deja fuera a quien sí opera efectos; es decisión del owner qué permiso lo gobierna.
  - 🔴 **Y el módulo de CONTRATOS no está desplegado en flor**: la tabla `contracts` no existe. El menú lo ofrecía igualmente a cualquiera con departamento → habría llevado a una pantalla rota. Para el scouter ya no se ofrece; para el resto sigue igual (pendiente decidir si se despliega o se oculta en esta instancia).

- **🪤 El primer scouter dado de alta NO VEÍA EL MÓDULO — dos puertas cerradas, una dentro de otra.** La regla de acceso por departamento estaba bien y la ruta lo dejaba pasar; lo que faltaba era el camino. (1) La barra lateral COMPLETA se pinta con `canSeePanel()` —al menos un permiso de panel—, y un scouter con rol `crew` no tiene ninguno: entraba y **no veía menú en absoluto**. (2) La SECCIÓN "Locaciones" del menú se pintaba con `locations.*`, y el enlace del Tech Scout vivía dentro. **Una puerta buena dentro de una puerta cerrada no sirve de nada.** Ahora pertenecer a Locaciones abre el panel y la sección. Guarda por los dos lados (el scouter SÍ la ve; quien no es de Locaciones NO). Lección: **poder entrar por la URL no es tener acceso** — si no está en el menú, para quien lo usa el módulo no existe, y una prueba que sólo hace `GET` de la ruta no lo detecta.

- **🪤 El barrido de nonce de la CSP borraba la VISTA de la respuesta — y con ella, tres pruebas de aislamiento.** `SecurityHeaders` estampa el nonce reescribiendo el HTML con `setContent()`, y ese método **pisa `$response->original`**, el objeto View que produjo la página. Consecuencia: cualquier prueba que mirara los datos de la vista de una página con `<script>` o `<style>` en línea moría con *"The response is not a view"*. Se llevó por delante `ReportOwnershipVisibilityTest` — justo las que vigilan que **un safety no vea el injury de otro**—, que llevaban rojas desde el cutover **sin que el rojo tuviera nada que ver con el aislamiento**. El daño no era la prueba: era quedarnos sin vigilancia sobre eso. Fix de dos líneas: guardar `original` y devolverlo tras reescribir. (Aislamiento comprobado intacto al volver el verde.)
  - Y **tres más en rojo por el cambio de autor a nombre de créditos** (mismo día, tranche anterior): las pruebas comparaban contra `users.name`. Ahora comparan contra `User::displayName()` —la misma función que estampa el documento—, para que la regla viva en un solo sitio. 🪤 **Se detectaron corriendo la suite COMPLETA**: las de módulo iban en verde y bastaron para dar por bueno el tranche.

### 2026-09-14/15 — 🔥 Primera jornada de USO REAL: 5 bugs de campo, una caída y el trabajo que casi se pierde
- **Estado:** ✅ **TODO EN PRODUCCIÓN Y VERIFICADO POR EL OWNER**, salvo el borrador de fotos (construido y probado; el owner lo valida en campo). Suite de documentos y de seguridad: **110/110 en verde**.
- **Contexto:** la app ya se usa en set, sobre iPad, mientras se camina la locación. Todo lo de abajo salió de usarla de verdad — nada habría aparecido en local.

- **🪤 LA LECCIÓN TRANSVERSAL DEL DÍA: el mismo dato en dos sitios siempre acaba divergiendo.** Tres bugs distintos resultaron ser la misma forma: título de pestaña, autor del documento y fotos del borrador. Cuando aparezca un dato duplicado, unificarlo ANTES de que se separen.

- **1 · La geolocalización "nunca funciona" — la rompimos nosotros.** `connect-src 'self'` bloqueaba los 3 servicios OSM de `crewcare-geo.js`. **El fallo no se ve como fallo:** el navegador SÍ da las coordenadas (la CSP no toca la geolocalización), sólo se bloquea la llamada que las traduce, y el código degrada a "escríbela a mano" — su conducta correcta sin internet. En pantalla parece un GPS roto. Roto desde el cutover a bloqueo; **`/csp-report` lo venía diciendo desde el primer día (54 violaciones en una jornada) y nadie leía ese log.** Fix: allowlist en `SecurityHeaders::GEO_CONNECT_SRC`. Guardia nuevo en `SecurityHeadersTest` (verificado: rompe si se vuelve a cerrar).
- **2 · Sólo se guardaban 20 imágenes.** No era la app: `max_file_uploads` (default **20**) descarta lo que sobra **en silencio**. 🪤 Y subirlo sin subir `max_input_vars` (default 1000) cambia un fallo visible por uno que **destruye datos**: ése cuenta CAMPOS, y al editar cada imagen manda 3; pasado el tope PHP recorta la cola y `update()` reconstruye el set con lo que llega → **borra imágenes ya guardadas**. Suben juntos o no suben. Ambos vigilados ahora por el preflight.
- **3 · Fotos "con zoom y pixeladas" SÓLO en iPad.** El documento se maqueta dentro de una **tabla** (`.report-wrap`, que el motor de paginación necesita para repetir cabecera y pie por hoja). Dentro de una celda, **WebKit resuelve el `height:100%` de una imagen ESTÁTICA contra el alto de la FILA** —una hoja entera—, la imagen se vuelve altísima, `object-fit:cover` la AMPLÍA y el `overflow:hidden` recorta un fragmento: se veía **el 1.5% de una foto de 2200×1650**. Chrome mide contra el contenedor → en escritorio, perfecto. Fix: `position:absolute; inset:0` (lo que ya hacía `.doc-hero .hero-bg`, la única imagen que siempre se vio bien en iPad — ésa fue la pista). ⚠ **Chrome en iPad ES WebKit**, así que "uso Chrome" no exime.
- **4 · 500 al auditar un vehículo.** `optional(null)->sortByDesc(...)` devuelve **NULL, no un Optional**: la cadena deja de ser segura tras el primer eslabón. Un vehículo al que le faltara UN documento tumbaba su propia ficha — y como el botón *Verificar* vive ahí, no se podía auditar ningún vehículo recién dado de alta.
- **5 · Rango de rodaje en el scouting.** `date_shoot_end` nullable (una locación se ocupa "del 14 al 17"). Se verificó **contra producción antes de escribir la migración**: una sola firma en toda la instancia, cero scoutings sellados → ventana abierta. Aun así entra en `NULLABLE_HASH_EXCLUDES`. El rankeo por fechas ahora cierra la ventana en el ÚLTIMO día (antes una locación de varios días dejaba de sugerirse desde el segundo).
- **6 · El autor se identifica por NOMBRE DE CRÉDITOS.** "Por: Ari" vs "Ari Rómulo" en la firma del **mismo** documento. La autofirma guardaba `->name` (sólo el nombre de pila), y **el nombre de pila deja de identificar en cuanto hay dos Genaros**. Sistémico: los 4 tipos con autofirma. Los documentos ya guardados muestran el crédito correcto sin reescribir nada sellado (las vistas resuelven desde `created_by_id`).
- **7 · 📸 SE PERDIÓ UN SCOUTING CON 20+ FOTOS al cerrar la pestaña.** La causa estaba escrita en nuestro código: `cc-drafts.js` guarda el texto pero **«el borrador NO captura `<input type=file>`»**. Las fotos vivían sólo en la memoria de la pestaña. Un scouting se llena **caminando**: rehacerlo es volver al sitio. Fix: **borrador EN SERVIDOR** (`scouting_drafts`) — cada foto viaja **al capturarse**, **en lote**, y al guardar viajan como RUTAS (el guardado final deja de mandar 30 MB). Sin red no se interrumpe la captura: se reintenta al volver, y si nunca vuelve va por el camino de siempre. Reusa el contrato de `vehicle_inspection_drafts`. 🔒 Cada ruta se comprueba contra el borrador DEL AUTOR (`ScoutingDraft::owns`): la ruta la propone el navegador. **Limitación conocida:** la última tanda, si la pestaña muere mientras sube, sí se pierde.
- **🪤 Un bug que el cambio anterior destapó:** la asignación de `additional_images_paths` vivía DENTRO del `if (hasFile(...))`. Con las fotos subiendo antes, el caso normal pasa a ser que **no llegue ningún archivo** al envío final → el scouting se habría guardado **sin ninguna imagen y sin error**.

- **🔥 EL SITIO SE CAYÓ A MEDIA JORNADA (3 min) — y la trampa que lo iba a repetir en CADA deploy.** El primer `composer install` se corrió como root → `vendor/` quedó `root:root`, la única carpeta que no era del usuario del sitio. Plesk despliega `artisan down` → `composer install` → `artisan up`; Composer no podía reescribir el autoloader, **el script abortaba y el `up` nunca corría**. Y lo peor no era la caída: con el autoloader congelado, **un commit con una clase nueva se despliega ROTO** (el archivo llega, PHP no lo encuentra) — los 6 commits pendientes de Salidas traen clases nuevas. Se resolvió **sin root**: `cp -R` de `vendor` (la copia nace del usuario) + dos `mv`. Todo en `DEPLOY-RUNBOOK.md §11·b` junto con la del `/usr/bin/php` sin `pdo_mysql`.
- **🛡️ Preflight 14→16** (`vendor/` escribible · no en modo mantenimiento) y el de subidas ampliado a `max_file_uploads`/`max_input_vars`. En prod: **0 fallas**.
- **🧪 Dos guardias de seguridad llevaban días en ROJO por razones conocidas** (`SecurityHeadersTest` daba por hecho el modo reporte; `TsaTimestampTest` esperaba un literal `'freeTSA'` que ya no emite nadie). **Un guardia que grita siempre está apagado:** el día que la CSP mató el motor geo, la suite ya no avisaba. Los dos reescritos para comprobar la REGLA, no un literal del entorno.
- **⚠ DISCIPLINA, dos veces en un día:** un test que pasa **no** prueba que el guardia funcione. Las dos pruebas de seguridad del borrador pasaban con `owns()` **desactivado** (usaban una clave sin borrador → la búsqueda devolvía null y nunca se llamaba). Igual que pasó con `NoMouseOnlyHandlersTest`. **Verificar SIEMPRE en los dos sentidos: romper el arreglo a propósito y comprobar que el test se pone rojo.**
- **Pendiente:** `vendor-root-viejo` (76 MB, necesita root) · los 6 commits de Salidas/Crew (otra sesión) · `capture` en `public/mitigation.blade.php` (decisión del owner: ahí forzar cámara tiene intención) · persistir localmente la última tanda de fotos.

### 2026-09-08/10 — 🚀 PRIMERA PRODUCCIÓN REAL (`flor.crewcare.mx`) + las 4 minas que cobró el primer día
- **Estado:** ✅ **EN CAMPO Y VERIFICADA.** 1ª instancia viva sobre VPS Plesk. El despliegue en sí tomó **3 días**, y casi nada de eso fue desplegar: fue descubrir en caliente cosas que en local no se ven. Todo lo aprendido quedó volcado en **`DEPLOY-RUNBOOK.md` §11** (léela ANTES del siguiente deploy) y en la memoria [[prod-flor-deploy]].
- **Verificado en prod:** migraciones al día · **CSP en BLOQUEO** (cabecera enforcing + nonce, cero violaciones) · correo real entregado · sello íntegro **con timbre TSA de DigiCert** · `crewcare:preflight` 0 fallas · `.env` no alcanzable por web (403/404).
- **🔑 MODELO DE RAMAS — una de despliegue POR INSTANCIA.** `upgrade/laravel-13` = desarrollo; **producción tira de `deploy/flor`**. Promover es deliberado (`merge` + push + *Pull now*). Sin esto, **cualquier commit de otra sesión entra al servidor del cliente en el siguiente pull**. 🪤 Ya pasó: la 1ª promoción arrastró un commit ajeno sin querer → **antes de empujar a `deploy/*`, listar los commits que van y confirmarlos.**
- **🪤 LAS 4 MINAS (todas corregidas):**
  1. **`config/app.php` tenía `'timezone' => 'UTC'` LITERAL** → `APP_TIMEZONE` del `.env` no la leía nadie. Un DSR sellado imprimía **6 h de más** y un reporte nació con la fecha de MAÑANA. **Un sello no se rehace.** Ahora lee el env y el default es `America/Mexico_City` (dato POR INSTANCIA, documentado con la tabla IANA de LATAM). El sello NO depende de la zona (verificado: mismo hash bajo UTC y CDMX). `phpunit.xml` ancla `APP_TIMEZONE=UTC` para que la suite no cambie de resultado según el país.
  2. **`mousedown` NO llega al tacto en iOS.** El typeahead —que usa TODO select largo— cableaba sus opciones sólo a `mousedown`: en iPad tocar una opción no corría `setVal()`, el `<select>` quedaba vacío y el server decía "campo obligatorio" **sin que nada estuviera vacío**. Parecía fallo de subida de archivos y costó horas. Fix: `pointerdown` (+ `mousedown` de respaldo); sigue siendo *down* para disparar antes del blur. Vigilado por **`NoMouseOnlyHandlersTest`** (verificado en los dos sentidos: con el bug REINTRODUCIDO el test rompe).
  3. **Tailwind NO está cargado en las vistas de reporte.** El relleno de la placa del logo lo daban `px-5 py-3` → clases inertes, relleno CERO, logo pegado a los bordes. En vistas de documento, **espaciado INLINE**.
  4. **`env()` es NULL con la config cacheada.** Con `config:cache` (el estado normal en prod) Laravel **no lee el `.env`**: `Preflight::checkSealKey()` usaba `env()` y gritó **[FALLA] "clave vacía"** en un servidor que la tenía bien puesta — falso positivo en el punto más delicado. **En comandos y servicios, SIEMPRE `config()`.**
- **Documentos — lo que se imprime:** el sello y el pie muestran **PUESTO** (`User::positionName()`), no el rol Spatie ni la etiqueta fija `label_risk_assessment` (que es el nombre del formato de Amazon, no el cargo de nadie). Se lee el puesto VIGENTE a propósito: una instancia = una producción y el puesto no cambia mientras dura (decisión owner; se descartó la columna de snapshot por complejidad sin beneficio). El hero ya no cae a `brand_name` (cuyo default es "CrewCare" → imprimía el nombre de la APP en el hueco del proyecto), sino al nombre de la producción.
- **Título de la pestaña — UNA sola fuente.** Salía de DOS sitios (login de `APP_NAME`, resto de `app_title`), y el que mandaba en el login era el `.env`, donde un valor con espacios **sin comillas** impide arrancar la app — el camino natural para arreglarlo era el que la tumbaba (el error de dotenv, *"unexpected whitespace"*, se lee como "no admite espacios" cuando significa "ponlo entre comillas"). Ahora todo usa `app_title`, y **se compone solo**: `CrewCare | <proyecto>` si nadie lo escribió. `APP_NAME` ya no manda en la interfaz.
- **`crewcare:preflight` pasó de 10 a 14 comprobaciones**, una por cada mina: zona horaria mal aplicada · Marca sin llenar · límites de subida para fotos desde el set · higiene (`APP_DEBUG`/`APP_ENV`/`APP_URL`). 🪤 `APP_ENV=local` es **AVISO y no falla**: es legítimo en la máquina del dev, y una alarma que grita en un caso válido entrena a ignorarla.
- **🔴 DEUDA CONOCIDA (no urgente, pero sépase):** el sello del DSR **NO cubre sus `daily_logs`** (el payload es `attributesToArray()` de `daily_reports`) → se pueden borrar/alterar hallazgos y el verificador sigue diciendo "íntegro". Y **borrar un log NO borra su foto**: los archivos quedan huérfanos en `storage/app/public/…` y su **URL es pública** (sólo el listado da 403) → toda limpieza tiene que borrar los ficheros.
- **Archivos:** `config/app.php`, `phpunit.xml`, `app/Console/Commands/Preflight.php`, `app/Support/Branding.php`, `resources/views/componentes/{_typeahead,_seal-cfdi,_doc-hero,_doc-hero-logo}.blade.php`, `resources/views/layouts/auth.blade.php`, `resources/views/admin/dailyreports/show.blade.php`, `resources/views/payee/intake.blade.php`, `tests/Feature/Security/NoMouseOnlyHandlersTest.php`, `DEPLOY-RUNBOOK.md`, `.env.example`. Migración `2026_09_08_000002` (`daily_logs.action_taken` → TEXT; widening, fuera de todo hash).
- **Riesgo/Notas:** `CREWCARE_SEAL_KEY` **no se rota jamás** con sellos vivos — el owner tiene copia del `.env` fuera del VPS (local + Drive); ⚠ si el respaldo de la BASE acaba en ese mismo Drive, la separación respaldo≠llave se pierde. Plesk despliega **copiando** (no hay `.git` en la raíz de la app) → para saber qué versión corre, comparar `md5sum` **normalizando CRLF** (`tr -d '\r'`), porque la copia de trabajo en Windows es CRLF y el servidor LF.

### 2026-08-11 — 🧱⬆️✅ Upgrade de framework COMPLETO: Laravel 8 → 13 (5 majors · PHP 7.4 → 8.3) — 475 verde en cada salto
- **Estado:** ✅ **COMPLETO — Laravel 13.25.0 / phpunit 11 / PHP 8.3.33**. Rama `upgrade/laravel-13`. Commits: `6447f9fe` (0·$dates), `6a2623f3` (1·L9), `d0e3e932` (2·L10), `2c942231` (data providers static), `42dfe35a` (3·L11), `6ec2aa4c` (4a·L12), `5ea79567` (4b·L13). **Suite 475 verde** en CADA compuerta (migrate:fresh+seed + view:cache + rutas), sobre PHP 8.1 (L9/L10) y **PHP 8.3** (L11). **PHP 8.3.33 TS x64 instalado en laragon** (`C:\laragon\bin\php\php-8.3.33-...`; ⚠ `extension_dir` ABSOLUTO + `zip`/`sodium` habilitados — el relativo rompía composer desde el repo).
- **Breakings resueltos por salto:** L9 = mail SwiftMailer→Symfony (solo un test), fuera fruitcake (CORS nativo)/laravelpwa/facade-ignition, quitado shim Blade @checked. L10 = alias Spatie `Middlewares`→`Middleware` (v6) + migración de permisos v6 (`PermissionRegistrar::$pivotPermission/$pivotRole` retiradas → se leen de config, **esquema idéntico**) + phpunit 9→10 (schema migrado). L11 = **CERO código** (data providers ya static; browsershot ^3.58→^5 forzado porque L11 usa symfony/process ^7).
- **L12** (`v12.66.0`, `6ec2aa4c`) y **L13** (`v13.25.0`, `5ea79567`) = **CERO código de app**. Holdouts de L13: `mailersend/laravel-driver` ^2→^3 y `laravel/tinker` ^2→^3 (las 2.x topaban en `illuminate/support ^12`); el resto ya soportaba L13.
- **Siguiente = SOLO Fase 5 (cierre):** verificación **VISUAL** del PDF (Browsershot 5 — re-chequear `savePdf`/fuentes inline) + cutover del sello **SHA→HMAC**. **Riesgo #1 (sello SHA):** INTACTO en los 5 saltos (475/475, incluidos los tests de detección de alteración).
...
- **Solución real = volver al MOTOR DE TABLA** (`report-wrap`: `<thead>`=hero y `<tfoot>` se REPITEN por hoja y reservan espacio, `@page{margin:0}` full-bleed, sin bandas). Ejecutado:
  ...
  - **Ambulancia** revertida al motor de tabla con cuerpo en FILAS (`941c99f4^`). Commit **`5a3c996c`**.
  - **Wrap** (el owner dio OK): convertido A MANO al motor de tabla preservando el editor (hero→`<thead>`, cuerpo→`<tbody>`, `<tfoot>` espaciador, hereda `@page{margin:0}`). El editor no depende del envoltorio → intacto. Commit **`88aaef18`**. **Ya ningún documento usa block-flow.**
  - **Historial médico** afinado (mismo commit `3b9e8d70`): tokens forzados a oscuro-sobre-blanco con `!important` (fin del "gris claro sobre blanco" en modo oscuro) + ocultar chrome/carets (fin de los "fragmentos de interfaz"). Verificado por CDP.
- **Aprendizaje duro (ver [[doc-hero-band-homologation]]):** para impresión multi-hoja usar SIEMPRE el motor de tabla; NUNCA flujo de bloques con pie `position:fixed`. El "corte" original era el bug Oficio-vs-Carta (ya resuelto en `fed2d2ed`), no el motor.
- **Estado:** TODOS los documentos (DSR + 6 + ambulancia + wrap + historial médico) = full-page/arreglados y pusheados. Ningún doc usa ya block-flow. **Falta sólo la confirmación VISUAL del owner al imprimir.** Prod pendiente sin cambios: `2026-08-09-permit-photos.sql` + `storage:link`.

### 2026-08-09 — (SUPERSEDIDA por la entrada de arriba) CORRECCIONES A y B — 1er intento
- **A) CORTE + BANDAS EN BLANCO en los reportes block-flow — RESUELTO (commit `f7e09475`).** Se diagnosticó con **PDF paginado real** (subagente + headless Chrome CDP `printToPDF{preferCSSPageSize:true}`; render CLI sin auth). La causa NO era el papel (todos ya son Carta desde `fed2d2ed`): el **pie de marca `position:fixed` (.print-foot, ~15mm)** tapaba/cortaba las últimas filas de CADA hoja porque **un `@page margin-bottom` NO reserva espacio de flujo a un elemento fijo** (prueba: apagando el pie, el contenido oculto reaparece). Las bandas en blanco venían del `@page{margin:12mm 0 15mm 0}`. **Fix:** `@page{size:letter;margin:8mm 0 18mm 0}` + `.doc-body{padding:4mm 12mm 0}` + `.print-foot{bottom:-16mm!important}` (empuja el pie a la zona del margen; ~2mm de blanco real bajo él). El `!important` es obligatorio (en `window.print()` real el JS pone `data-view="print"` y el head trae `:root[data-view="print"] .print-foot{bottom:0}` más específico). Verificado: bottom computado = -16mm en scouting/wrap/DSR con data-view=print. Aplicado en las 3 copias del CSS: `_report-v2-blockflow` (scouting/DSR/injury/hazard/unsafecond/PAE/inspección-acta), `admin/wrap/show`, `ambulance/acta`. Ver [[doc-hero-band-homologation]] gotcha #3 (RESUELTO).
- **B) HISTORIAL MÉDICO: imprimir la vista real, no un doc chrome-v2 — RESUELTO (commit `ee5c0a43`).** Se retiró la vista standalone `componentes/historiamr-print` que un subagente había creado (`1abaf634`) + su ruta `historialwr.imprimir` + `cmedicController@historialImprimir`. El botón "Imprimir" vuelve a `window.print()` sobre `#hm-report`. Se rehízo su `@media print`: fuera el `body *{visibility:hidden}` + `position:absolute` (el absoluto rompía la paginación → Chrome recortaba a la 1ª hoja); ahora en flujo, a todo el ancho (reset col-md-10 + min-height:0; la app ya oculta el chrome con `.no-print`), `@page{size:letter;margin:14mm 12mm}`, tablas/sellos atómicos con break-inside:avoid. Ver [[health-record-module]].
- **Estado:** ambas hechas, verificadas y **pusheadas** (`ee5c0a43`, `f7e09475`). Falta sólo la confirmación VISUAL del owner al imprimir (no puedo ver la impresión real desde aquí; sí verifiqué con PDF real + cascada de window.print()). Prod pendiente sin cambios: `2026-08-09-permit-photos.sql` + `storage:link`.
...
### 2026-08-24 — 🚚 TRANSPORTACIÓN · Bloque 1 · AJUSTES (borrador, licencia con la persona, is_towed, HEIC global)
- **Estado:** Hecho (suite **815 verde**, +5 tests). 4 cambios aditivos sobre el Bloque 1.
- **§1 Borrador del checklist:** tabla NUEVA `vehicle_inspection_drafts` (mig `2026_08_24_000015` + owner-apply `2026-08-24-transport-drafts.sql`) + modelo `VehicleInspectionDraft`. Guardado parcial EN SERVIDOR, DEL AUTOR, retomable desde cualquier dispositivo; fotos al vuelo, se reusan al sellar y el borrador se BORRA. Botón "Guardar borrador" + completitud por módulo en `execute`. Rutas `transport.inspect.draft(.discard)`. El acta sigue inmutable.
- **§2 Licencia con la PERSONA:** `VEH_LICENCIA` sale de los docs del vehículo (`Vehicle::REQUIRED_DOC_CODES` 4→3) y vive en el paquete del driver (payee, holder=Payee level persona, `storeDriverLicense`). La ficha la MUESTRA read-only (`Vehicle::driverLicense()`); un driver con 2 unidades = 1 licencia. NO cambia el veredicto (los docs nunca lo alimentan); el badge "docs revisados" = 3 docs del vehículo.
- **§3 `is_towed`:** atributo nuevo (≠ `tows`). Evaluador `applies_when` ahora soporta `&&`/`||`. Excluye 14 puntos de núcleo de conducción/propulsión en unidades remolcadas; REMOLQUE ampliado a `tows||is_towed` + **REM-004 nuevo** (frenos del remolcado). Energía/habitables/agua siguen. Catálogo 50→**51** (re-seed, sin SQL de esquema).
- **§4 HEIC global:** `public/js/cc-photo-auto.js` idempotente + engancha todo `input[type=file][accept*=image]:not([data-cc-noauto])`, cargado en el layout (todas las páginas). scouting/riskmap marcados `data-cc-noauto`. iPhone/iPad convierten antes de subir. **`libheif` = requisito del host** (respaldo del servidor).
- **Archivos tocados que ya existían:** `Vehicle` (REQUIRED_DOC_CODES + driverLicense), `VehicleController` (draft + license), `routes/web.php`, `execute`/`vehicle-show`/`lite` blades, `VehicleChecklist` (&&/||), catálogo JSON, `layouts/app.blade.php` (+scripts globales), `scoutings/_form` + `riskmaps/edit` (`data-cc-noauto`).
- **Riesgo/Notas:** ver [[transport-vehicle-module]] + [[pendientes-prod]] 4g. Fresh = mig 000015 + re-seed; prod existente = 1 SQL (drafts) + re-seed catálogo. HEIC: host necesita Imagick+libheif.

### 2026-08-24 — 🚚 TRANSPORTACIÓN · Bloque 1 (entidad Vehículo + verificación de seguridad)
- **Estado:** Hecho (suite **810 verde**, +23 tests nuevos). Módulo NUEVO, todo aditivo.
- **Archivos (nuevos):** migraciones `2026_08_24_000010..000014` (vehicle_types/vehicle_check_points/vehicles/vehicle_inspections + puente `payee_contracts.vehicle_id`); gemelos `database/owner-apply/2026-08-24-transport-catalog.sql` + `-vehicles.sql`; modelos `Vehicle`/`VehicleType`/`VehicleCheckPoint`/`VehicleInspection`; soportes `VehicleVerdict`/`VehicleChecklist`/`TransportAccess`; `VehicleController`; seeders `VehicleCatalogSeeder`/`TransportPermissionsSeeder`/`TransportBackfillVehiclesSeeder` + data `crewcare_transporte_catalogo.json`; vistas `resources/views/vehicle/*` (HUB, flota, ficha, edición, execute, acta, rejection, records, lite, _marks, _form-fields); tests `tests/Feature/Transport/*`.
- **Archivos (tocados que ya existían):** `SealVerifier::TYPES` (+1 línea `'veh'`); `DocumentTypeSeeder` (+4 tipos `VEH_*`); `DatabaseSeeder` (+2 seeders en el chain); `routes/web.php` (grupo `/transportacion`); `sidebar.blade.php` (**sección PROPIA "Transportación"**, desktop+móvil); `Vehicle`/`payee_contracts` puente.
- **Qué / Por qué:** verificación de seguridad del vehículo en prep. Veredicto GRADUADO (critical/major/minor → nivel interno alto_riesgo..excelente → apto/no_apto cara pública), fotos OBLIGATORIAS por punto, `applies_when` sobre atributos que encienden módulos del checklist, acta sellada HMAC verificable (`'veh'`, sin filtrar el nivel), reevaluación encadenada, 2 marcas de badge (inspeccionado/docs revisados), vista lite de producción. Promueve `payee_contracts.asset_ref` a entidad `vehicles`.
- **Decisiones del owner:** RBAC HÍBRIDO — `transport.manage` (safety+super-admin) + `transport.view` (producción lite) + el **departamento de Transportación levanta el checklist por pertenencia** (no edita actas). **Transpo ≠ Seguridad** aun del checklist → sección propia en el sidebar. FUERA a propósito: órdenes de transportación, bitácoras combustible/km, picture cars (permiso `vehiculo_escena`), maquinaria (herramientas).
- **Riesgo/Notas:** columna de atributos = **`attr_values`** (NO `attributes`, colisión Eloquent). Fresh install = migraciones + chain automático; prod existente = 2 SQL + 3 seeders (ver [[pendientes-prod]] 4f). ⚠ HEIC con fotos obligatorias: host sin libheif rechaza (usuario sube JPG; iPad convierte en navegador). Ver [[transport-vehicle-module]].

### 2026-08-24 — 🧭 Decisiones de dirección (mapa vs. visión de junio) + roadmap de traspaso
- **Estado:** Planeación (sin código). Traza para no perder rumbo.
- **Qué/Por qué:** revisado el mapa contra la visión de junio (`VISION-PRODUCT.md`, 24-jun) y el `ROADMAP.md`. Fases 0-2 completas; **estamos a mitad de la Fase 3** (operación de set). Decisiones del owner:
  - **Breakdown de guion + Sides → FUERA a propósito de esta etapa** (confidencialidad + no competir de frente con Scenechronize; primero servicio seguro e íntegro). Sigue siendo meta a futuro, no es hueco olvidado.
  - **Próximo módulo = Transportación** (vehículo como entidad de 1ª clase + verificación de seguridad + órdenes driver+vehículo, alimentadas por los pick-ups del Llamado). El owner lo **aterriza en Claude Chat** antes de construir. Gancho existente: `payee_contracts.asset_ref`.
  - **Track paralelo de seguridad/legalidad (lo hace Claude Code):** **TSA RFC 3161 con freeTSA.org** sobre todos los sellos (timbra el HASH, best-effort/asíncrono; solo el hash viaja → confidencial) + endurecer BD/app (cifrado en reposo, MFA, bitácora de acceso, HTTPS/CSP, backups). "Superior a SHA" = **HMAC (ya hecho)**; siguiente = TSA + posible firma asimétrica.
  - **Sello del DSR:** decidido → **solo se sella el documento final** (no cada evento/`daily_log`) — cierra esa deuda.
- **Riesgo/Notas:** sin cambios de código ni esquema. Todo volcado a memoria (`security-hardening-and-tsa-track`, `audit-loose-ends-2026-08`) + índice de memoria compactado (19 módulos cerrados movidos al histórico). Roadmap de traspaso a Claude Chat entregado en el chat (no persistido a archivo por ahora).

### 2026-08-24 — 📤 Envío con marca de agua + Distribución · 🚀 commit/push del backlog del Llamado
- **Estado:** Hecho (suite completa **787 verde**; rama `upgrade/laravel-13` **subida a origin**).
- **Archivos (nuevos):** `app/Support/PdfWatermarker.php`, `app/Support/WatermarkFpdi.php`, `app/Support/FileDeliveryDispatcher.php`; `app/Models/FileDelivery.php`, `app/Models/FileDeliveryRecipient.php`; `app/Console/Commands/DispatchFileDeliveries.php`; `app/Http/Controllers/FileDeliveryController.php`; migraciones `2026_08_24_000002_create_file_deliveries` + `2026_08_24_000003_add_extra_docs_to_call_packages`; `database/owner-apply/2026-08-24-file-deliveries.sql`; vistas `admin/deliveries/{index,create,show}.blade.php` + `correos/file-delivery.blade.php`; `tests/Feature/Deliveries/FileDeliveryTest.php`.
- **Archivos (tocados):** `CallSheetController` (`sendToCrew`→outbox+ráfaga inline+guardas; `uploadExtra`/`removeExtra`; `buildMergedPdf` une adicionales tras el back), `Console/Kernel` (agenda `deliveries:dispatch` cada minuto), `User` (`creditShortName`: créditos o primer nombre+primer apellido), `config/features` (`callsheet_extra_docs` OFF) + `FeatureFlagController`, `routes/web`, `layouts/sidebar` (link Distribución), `componentes/_icon`, `DEPLOY-RUNBOOK.md`, `database/README.md`.
- **Qué/Por qué:** afinar el envío masivo del llamado → subsistema **REUSABLE** de envío con marca de agua por persona. El envío **no bloquea ni se pierde**: outbox (`file_deliveries`+`file_delivery_recipients`) + **ráfaga inline** al enviar + **cron** `deliveries:dispatch` (`schedule:run`) que drena en tandas con reclamo atómico. Cada copia lleva el **nombre en créditos** en una sola línea diagonal tenue (`PdfWatermarker`/FPDI conserva el texto), como los llamados reales. Mismo motor sirve al módulo general **Distribución** (`/distribucion`, a todos los usuarios activos, gate `settings.manage`). PDF adicional al paquete tras flag. La marca **nunca** toca el congelado/firmado (se aplica a la copia de cada quien al enviar). Detalle: memoria `file-delivery-watermark-module`.
- **Commits/push (traza):** hasta hoy **no se había commiteado nada desde `2fdfcd61` (21-ago)** ni pusheado la rama (origin solo tenía `main`/`clean-main`). Se subió TODO el backlog acumulado a `origin/upgrade/laravel-13` en 5 commits agrupados: `468fa862` Llamado (motor+back+paquete) · `6e6e3a8f` envíos con marca de agua+Distribución · `10785d1b` visibilidad de reportes+sueltos · `7ab23640` docs · `75cc75a8` chore (caché de fuente). El VPS clonará `-b upgrade/laravel-13`.
- **Riesgo/Notas:** 🔴 **DEPLOY — requisito nuevo permanente: el cron `schedule:run`** (`DEPLOY-RUNBOOK.md` §5·b); sin él los envíos masivos no se completan solos (solo sale la ráfaga inline ~20). El esquema entra solo con `migrate --force` en deploy fresco (los owner-apply gemelos son para BD ya poblada). Guardas `Schema::hasTable`/`hasColumn` como **red de seguridad** (no sustituyen un deploy íntegro: con `git clone` + `migrate` las tablas SÍ existen). `.env` no rastreado (verificado). Suite **787 verde** antes del push.
