...
- **Solución real = volver al MOTOR DE TABLA** (`report-wrap`: `<thead>`=hero y `<tfoot>` se REPITEN por hoja y reservan espacio, `@page{margin:0}` full-bleed, sin bandas). Ejecutado:
  ...
  - **Ambulancia** revertida al motor de tabla con cuerpo en FILAS (`941c99f4^`). **Wrap**: se quitó mi pie hundido (regresión); sigue en block-flow por ahora (hospeda el editor — conversión pendiente, a confirmar con el owner). Commit **`5a3c996c`**.
...
- **Estado:** DSR + 6 + ambulancia + historial médico = full-page/arreglados y pusheados. **Pendiente:** conversión del **wrap** a full-page (con cuidado del editor) — esperando OK del owner. Falta la confirmación VISUAL del owner al imprimir.
